...
select c.CAT_ID as Category_Id, c.CAT_NAME as Category_Name, p.PRO_NAME as Product_Name, s.SUPP_PRICE as Price FROM category c INNER JOIN product p ON p.CAT_ID = c.CAT_ID INNER JOIN supplier_pricing s ON s.PRO_ID = p.PRO_ID WHERE s.SUPP_PRICE = (SELECT MIN(s2.SUPP_PRICE) FROM supplier_pricing s2 WHERE s2.PRO_ID = s.PRO_ID);
...
BEGIN
SELECT s.SUPP_ID as Supplier_Id, s.SUPP_NAME as Supplier_Name, avg(r.RAT_RATSTARS) as Average_Rating,
CASE
...
group by s.SUPP_ID, s.SUPP_NAME, r.RAT_RATSTARS
order by s.SUPP_ID;
END //
DELIMITER ;
